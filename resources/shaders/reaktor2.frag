//http://glsl.heroku.com/e#11376.0
...
#define N2(p) t(O_,M_,p) + t(M_,J_,p) + t(G_,I_,p) + t(I_,L_,p) + t(L_,J_,p)
#define IU(p) t(D_,F_,p) + t(E_,N_,p) + t(M_,O_,p)
...
d += N(r()); add();
space();
d += IU(r()); add(); caret.x -= 0.1;
d += IU(r()); add();

	d = clamp(d* (.75+sin(gl_FragCoord.x*3.1456+time*1.3)*.5), 0.0, 1.0);
      
    col = vec3(d*.5, d, d*.85);  
    gl_FragColor = vec4( col, col.b ); // Write blue into alpha channel so that it is transparent when the colour is black
}
